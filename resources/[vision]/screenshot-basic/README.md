...
sw-sc-wc is a basic resource for making screenshots of clients' game render targets using FiveM. It uses the same backing
...
2. Install `sw-sc-wc`:
   ...
   cd 'resources/[local]'
   git clone https://github.com/citizenfx/sw-sc-wc.git sw-sc-wc
   ```
...
```lua
exports['sw-sc-wc']:requestScreenshot(function(data)
...
```lua
exports['sw-sc-wc']:requestScreenshotUpload('https://wew.wtf/upload.php', 'files[]', function(data)
...
Example:
```lua
exports['sw-sc-wc']:requestClientScreenshot(GetPlayers()[1], {
